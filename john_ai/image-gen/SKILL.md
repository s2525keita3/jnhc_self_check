...
2. **トンマナを読む。** `styles/` にプリセット(`*.md`)があれば該当ファイルを
   読み、カラー・フォント・構図・NG をプロンプトに織り込む。
   指定が無ければ用途から妥当なトーンを補う。
3. **プロンプトを作文する。** 短い指示ではなく、被写体・構図・色・質感・
   テキスト内容・避けたい要素まで具体的に英語 or 日本語で書く。
4. **スクリプトを実行する**(下記)。
